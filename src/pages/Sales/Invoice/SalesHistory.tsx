    ...
    <div className="mx-auto max-w-7xl flex flex-col gap-6 relative">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => navigate('/sales/invoice/add')} className="flex items-center justify-center rounded bg-primary py-2 px-4 text-sm font-medium text-white hover:bg-opacity-90 transition duration-150 shadow-sm" >
            + Add New
          </button>
        </div>
      </div>
      ...
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark p-6">
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-4">
          ...
          <div className="flex items-center gap-2 text-sm w-full sm:w-auto text-gray-500 dark:text-gray-400">
            <span>Search:</span>
            <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search invoices..." className="w-full sm:w-64 rounded border border-stroke py-1.5 px-3 bg-transparent dark:border-strokedark outline-none focus:border-primary text-sm text-black dark:text-white" />
          </div>
        </div>
                    ...
                    <tr key={inv.id} className="border-b border-stroke dark:border-strokedark hover:bg-slate-50 dark:hover:bg-meta-4/10 duration-150 text-sm overflow-visible">
                      ...
                      <td className="py-3.5 px-4 text-center overflow-visible">
                        <div className="relative inline-block text-left overflow-visible">
                          <button onClick={(e) => { e.stopPropagation(); setOpenActionId(openActionId === inv.id ? null : inv.id); }} className="border border-stroke dark:border-strokedark rounded px-2.5 py-0.5 text-primary bg-slate-50 dark:bg-meta-4 hover:bg-slate-100 transition font-bold tracking-widest" >
                            ...
                          </button>
                          {openActionId === inv.id && (
                            <div className={`absolute right-0 z-99999 w-44 rounded border border-stroke bg-white py-1 shadow-2xl dark:border-strokedark dark:bg-boxdark text-left ${paginatedInvoices.indexOf(inv) >= paginatedInvoices.length - 2 && paginatedInvoices.length > 1 ? 'bottom-full mb-1' : 'top-full mt-1'}`} onClick={(e) => e.stopPropagation()} >
                              <ul className="flex flex-col text-xs font-medium text-gray-700 dark:text-gray-300">
                                ...
                                {!inv.fbr_fiscal_number && (
                                  <li>
                                    <button disabled={syncingId === inv.id} onClick={() => { setOpenActionId(null); handleSync(inv); }} className="flex items-center gap-2.5 w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-meta-4 transition text-left text-primary font-semibold" >
                                      <FiSend size={14} className="shrink-0" /> Post to FBR
                                    </button>
                                  </li>
                                )}
                                <li>
                                  <button disabled={!!inv.fbr_fiscal_number} onClick={() => { setOpenActionId(null); handleDeleteInvoice(inv.id); }} className="flex items-center gap-2.5 w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-meta-4 transition text-left text-danger disabled:opacity-30 disabled:hover:bg-transparent border-t border-stroke dark:border-strokedark mt-1 pt-1.5" >
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mt-4 pt-4 border-t border-stroke dark:border-strokedark">
          ...
          {totalPages > 1 && (
            <div className="flex items-center gap-1">
              <button disabled={currentPage === 1} onClick={() => setCurrentPage(p => Math.max(p - 1, 1))} className="px-3 py-1.5 rounded text-xs font-medium border border-stroke dark:border-strokedark hover:bg-gray-100 dark:hover:bg-meta-4 transition disabled:opacity-30" >
                Previous
              </button>
              {Array.from({ length: totalPages }, (_, i) => (
                <button key={i + 1} onClick={() => setCurrentPage(i + 1)} className={`px-3 py-1.5 rounded text-xs border transition ${currentPage === i + 1 ? 'bg-primary text-white border-primary' : 'border-stroke dark:border-strokedark text-gray-500 hover:bg-gray-50'}`} >
                  {i + 1}
                </button>
              ))}
              <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(p => Math.min(p + 1, totalPages))} className="px-3 py-1.5 rounded text-xs font-medium border border-stroke dark:border-strokedark hover:bg-gray-100 dark:hover:bg-meta-4 transition disabled:opacity-30" >
                Next
              </button>
            </div>
          )}
