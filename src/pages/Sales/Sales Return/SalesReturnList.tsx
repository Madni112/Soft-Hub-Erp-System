...
const SalesReturnList = () => {
    ...
    // TEST INJECTOR FUNCTION: Dispatches programmatic sales return entry directly into Supabase
    const handleTestInjectReturn = async () => {
        try {
            const testReturnPayload = {
                original_invoice_no: "1",
                customer_name: "Bismillah Gas Supplier",
                return_date: new Date().toISOString().split('T')[0],
                return_type: "On Credit",
                salesman: "Madni Team Leader",
                remarks: "Defective commercial batch return test logs",
                total_quantity: 2.00,
                total_amount: 200.00,
                total_gst_amount: 36.00,
                total_net_amount: 236.00,
                items: [
                    {
                        itemName: "Gas Cylinder Compound 45KG",
            ...
            const { data, error } = await supabase
                .from('sales_returns')
                .insert([testReturnPayload])
                .select();

            if (error) throw error;

            toast.success('Test Sales Return injected successfully into Supabase logs!');
            fetchSalesReturns();
        } catch (err: any) {
            toast.error('SQL Return Insertion Failure: ' + err.message);
        }
    };
    ...
    // Live filter evaluation parameter mappings
    ...
    // Pagination bounds vectors computation math loops
    ...
    return (
        <div className="mx-auto max-w-7xl flex flex-col gap-6 relative">
            ...
            {/* HEADER TITLE ROW WITH + ADD NEW BUTTON PLACED TOP RIGHT */}
            <div className="flex items-center justify-between">
                <h2 className="text-xl font-bold text-black dark:text-white flex items-center gap-2">
                    Sales Return / Debit Notes
                </h2>

                {/* TOP RIGHT ALIGNED CONTROL GRID CONFIG */}
                <div className="flex items-center gap-3">
                    <button
                        type="button"
                        onClick={handleTestInjectReturn}
            ...
            {/* MAIN DATATABLE CANVAS INTERFACE BOX */}
            <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark p-6">

                {/* Top Filters Handling Configuration Row */}
                    ...
                    <table className="w-full table-auto border-collapse">
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={8} className="text-center py-10 text-sm text-gray-500 dark:text-gray-400">
                                    ...
                                    return (
                                        <tr key={ret.id} className="border-b border-stroke dark:border-strokedark hover:bg-slate-50 dark:hover:bg-meta-4/10 duration-150 text-sm">
                                            ...
                                            <td className="py-3.5 px-4 font-bold text-primary dark:text-white">
                                                {`RTN-${String(ret.id).padStart(4, '0')}`}
                                            </td>
                                            <td className="py-3.5 px-4 font-medium text-gray-600 dark:text-gray-400">
                                                {ret.original_invoice_no ? `INV-${String(ret.original_invoice_no).padStart(4, '0')}` : '-'}
                                            </td>
                                            <td className="py-3.5 px-4 text-gray-500 whitespace-nowrap">
                                                {ret.return_date ? ret.return_date : new Date(ret.created_at).toLocaleDateString()}
                                            </td>
                          ...
                          ${ret.return_type === 'Cash' ? 'bg-success' : 'bg-danger'}`}>
                                                    {ret.return_type || 'On Credit'}
                                                </span>
                                            </td>
                                            <td className="py-3.5 px-4 text-right font-bold text-black dark:text-white">
                                                {Number(ret.total_net_amount || ret.total_amount || 0).toFixed(2)}
                                            </td>

                                            {/* DYNAMIC INTEGRATED INLINE ROW ACTION CONTROL BLOCK BAR */}
                                            <td className="py-3.5 px-4 text-sm">
                                                <div className="flex items-center justify-center space-x-3">
                                                    <button
                                                        type="button"
                                                        onClick={() => navigate(`/Sales-Return/Debit-Notes/Print/${ret.id}`)}
                                                    ...
                                                    <button
                                                        type="button"
                                                        onClick={() => navigate('/Sales-Return/Debit-Notes/Add', { state: { returnRecord: ret } })}
                ...
                {/* DYNAMIC BOTTOM STATISTICS ENTRY DATA REPORTING STRIP AND NEXT/PREV NAVIGATION BUTTONS ARRAY */}
