...
const AddSalesReturn = () => {
  ...
  // Route state check to determine if in Edit Mode for a previously saved Return
  ...
  // Hydrate form if editing a saved Return record
  useEffect(() => {
  ...
  // AUTOMATIC LOOKUP FUNCTION: Triggers when user enters or loses focus on Original Invoice No
  const handleInvoiceLookup = async (invoiceId: string, setFieldValue: any) => {
    ...
    try {
      ...
      if (invData) {
        ...
        // Map item rows and add a temporary max qty track block
        const mappedItems = (invData.items || []).map((item: any) => ({
          ...
          qty: Number(item.qty) || 1, // Original sold quantity constraint boundary
          returnedQty: Number(item.qty) || 1 // Defaults to full return
        }));
      ...
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        ...
        <Formik
          ...
          onSubmit={async (values) => {
            ...
            // Re-calculate totals matching FBR Pakistan tax pathways for returns
            let totalQty = 0;
          ...
          {({ values, handleChange, setFieldValue }) => (
            <Form className="p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-x-6 gap-y-4 mb-6 text-sm">
                ...
                {/* Field 2: AUTOMATIC INTERACTIVE LOOKUP TRIGGER FIELD */}
                <div>
                  ...
                  <div className="relative flex">
                    <input 
                      ...
                      onBlur={(e) => handleInvoiceLookup(e.target.value, setFieldValue)}
                      value={values.originalInvoiceNo} 
                      className="w-full rounded border border-stroke p-2 bg-transparent dark:border-strokedark outline-none focus:border-primary text-xs font-bold" 
                      placeholder="Type Invoice ID (e.g. 12)"
                      disabled={isEditMode}
                      required
                    />
                    {fetchingInvoice && (
                ...
                {/* Field 3: Read-Only Customer info auto-filled by lookup row */}
                <div>
                ...
                {/* Field 4: Return execution transaction timestamp date code */}
                <div>
                ...
                {/* Field 5: Return balance compensation classification */}
                <div>
                ...
                {/* Field 6: Salesman auto-populated value */}
                <div>
                ...
                {/* Field 7: Core Scenario type text indicator */}
                <div className="md:col-span-2">
                  ...
                  <input type="text" name="scenarioType" readOnly value={values.scenarioType} className="w-full rounded border border-stroke p-2 bg-gray-100 dark:bg-meta-4 outline-none text-xs text-gray-500 truncate" placeholder="Auto-filled target profile layout schema" />
                </div>
              </div>

              {/* ===== LINE ITEM INTERACTION LOG TABLES CANVAS ===== */}
                        ...
                        {values.items.map((item: any, index: number) => {
                          ...
                          return (
                            <tr key={index} className="border-b border-stroke dark:border-strokedark">
                              ...
                              {/* Editable interaction cell box: User types how many items are being returned */}
                              <td className="p-1 border-r border-stroke dark:border-strokedark bg-blue-50/20">
                                <input 
                                  ...
                                  onChange={(e) => {
                                    ...
                                    // Constraint validation: block returns greater than original sold quantity values
                                    if (val > soldQty) {
                                      toast.error(`Cannot return more than originally sold (${soldQty})`);
                                      setFieldValue(`items.${index}.returnedQty`, soldQty);
                                    } else {
                                      handleChange(e);
                                    }
                                  }} 
                                  value={item.returnedQty} 
                                  className="w-20 p-1 border border-primary rounded text-center font-bold bg-transparent text-primary outline-none" 
                                  disabled={!values.originalInvoiceNo}
                                />
                              </td>
              ...
              {/* ===== SECTION 3: CALCULATION TOTAL SUMMARY CARD ===== */}
              <div className="flex flex-col md:flex-row justify-between items-start gap-6 mt-6 border-t border-stroke dark:border-strokedark pt-6">
                ...
                <div className="w-full md:w-1/3 space-y-2 text-xs border border-stroke dark:border-strokedark rounded-sm p-4 bg-gray-50/50 dark:bg-meta-4/10">
                  <div className="flex justify-between border-b pb-1.5 dark:border-strokedark">
                    ...
                    <b className="text-danger text-sm font-bold">
                      {values.items.reduce((sum: number, i: any) => sum + (Number(i.returnedQty) || 0), 0)}
                    </b>
                  </div>
                  <div className="flex justify-between border-b pb-1.5 dark:border-strokedark">
                    ...
                    <b className="text-black dark:text-white text-sm">
                      {(() => {
                        const isThirdSchedule = values.scenarioType === "Sale of 3rd Schedule Goods";
                        return values.items.reduce((sum: number, i: any) => {
                          const base = isThirdSchedule ? Number(i.mrp) : Number(i.rp);
                          return sum + (base * (Number(i.returnedQty) || 0));
                        }, 0).toFixed(2);
                      })()}
                    </b>
                  </div>
                  <div className="flex justify-between border-b pb-1.5 dark:border-strokedark">
                    ...
                    <b className="text-black dark:text-white text-sm">
                      {(() => {
                        ...
                        return values.items.reduce((sum: number, i: any) => {
                          const base = isThirdSchedule ? Number(i.mrp) : Number(i.rp);
                          return sum + ((base * (Number(i.returnedQty) || 0) / 100) * (Number(i.gstRate) || 0));
                        }, 0).toFixed(2);
                      })()}
                    </b>
                  </div>
                  <div className="flex justify-between pt-1 text-danger font-extrabold text-base uppercase tracking-tight">
                    ...
                    <span>
                      {(() => {
                        ...
                        return values.items.reduce((sum: number, i: any) => {
                          const base = isThirdSchedule ? Number(i.mrp) : Number(i.rp);
                          const amt = base * (Number(i.returnedQty) || 0);
                          const tax = (amt / 100) * (Number(i.gstRate) || 0);
                          const ftax = (amt / 100) * (Number(i.fTaxPer) || 0);
                          return sum + (amt + tax + ftax);
                        }, 0).toFixed(2);
                      })()}
                    </span>
                  </div>
              ...
              {/* ===== FORM ACTION FOOTER SUBMIT CONTROLS ===== */}
              <div className="flex items-center gap-4 border-t border-stroke pt-5 mt-6 dark:border-strokedark">
                <button
                  type="submit"
                  disabled={loading || !values.originalInvoiceNo}
                  className="rounded bg-success py-2.5 px-10 font-medium text-white hover:bg-opacity-90 transition disabled:opacity-40 text-sm shadow-xs"
                >
